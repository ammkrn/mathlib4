...
instance : AddSemigroup (Fin UInt8.size) where
  add_assoc := Fin.add_assoc

instance : AddMonoid (Fin UInt8.size) where
  add_zero := Fin.add_zero
  zero_add := Fin.zero_add
  nsmul := Fin.nsmul
  nsmul_zero' := Fin.zero_mul
...
instance : AddCommMonoid (Fin UInt8.size) where
  add_comm := Fin.add_comm

instance : SubNegMonoid (Fin UInt8.size) where
  sub_eq_add_neg := Fin.sub_eq_add_neg
  gsmul := Fin.gsmul
  gsmul_zero' := Fin.gsmul_zero'
  gsmul_succ' := Fin.gsmul_succ'
...
instance : AddGroup (Fin UInt8.size) where
  add_left_neg := Fin.add_left_neg

instance : Monoid (Fin UInt8.size) where
  mul_one := Fin.mul_one
  one_mul := Fin.one_mul
  mul_assoc := Fin.mul_assoc
  npow_zero' := fun _ => rfl
...
instance : CommMonoid (Fin UInt8.size) where
  mul_comm := Fin.mul_comm

instance : MonoidWithZero (Fin UInt8.size) where
  zero_mul := Fin.zero_mul
  mul_zero := Fin.mul_zero
